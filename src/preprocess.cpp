...
void Preprocess::my_avia_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  // pcl::PointCloud<PointCloudXYZI> pl_orig;
  sensor_msgs::PointCloud pl_orig;
	sensor_msgs::convertPointCloud2ToPointCloud(*msg, pl_orig);
  // float intensity,tag,line;
  ...
  //       << endl;
	...
	// }
  // for(int i=0;i<pl_orig.channels.size();i++)
  // {
  //   cout<< "pl_orig.channels["<<i<<"].name: "<<pl_orig.channels[i].name<<std::endl;
  //   cout<< "pl_orig.channels["<<i<<"].values.size(): "<<pl_orig.channels[i].values.size()<<std::endl;
  // }
	// cout << "------" << endl;
  ...
  uint valid_num = 0;
  {
    ...
    {
      if((pl_orig.channels[2].values[i] < N_SCANS) && (((int)pl_orig.channels[1].values[i] & 0x30) == 0x10 || ((int)pl_orig.channels[1].values[i] & 0x30) == 0x00))
      {
        // 	cout << pl_orig.points[i].x << ", " << pl_orig.points[i].y << ", " << pl_orig.points[i].z 
        // // << endl;
		    //  <<", intensity: "<< pl_orig.channels[0].values[i]
        ...
        if (valid_num % 1 == 0)
        {
          ...
          // cout<<"asdfgasdf2\n";
          pl_full[i].intensity = pl_orig.channels[0].values[i];
          pl_full[i].curvature = 0.0; // use curvature as time of each laser points, curvature unit: ms
          // cout<<"asdfgasdf3\n";
...
void prin_data_attr(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  float start_time = sensor_msgs_pl_orig.header.stamp.sec + sensor_msgs_pl_orig.header.stamp.nsec*1e-6;
  cout<< "start_time: "<<start_time<<std::endl;
  ...
  for (int i=0; i<sensor_msgs_pl_orig.points.size(); i++) {
    ...
    <<", timestamp: "<< sensor_msgs_pl_orig.channels[1].values[i]-start_time
